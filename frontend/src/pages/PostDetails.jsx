...
// --- ICON IMPORTS ---
import { MdArrowBack, MdSave, MdDeleteForever } from "react-icons/md";
import { BiSolidEditAlt } from "react-icons/bi";
import { RiImageEditFill } from "react-icons/ri";
import { BsThreeDotsVertical } from "react-icons/bs";
...
const PostDetails = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const pRes = await sendRequest(`${import.meta.env.VITE_BACKEND_URL}/api/posts/${postId}`);
        setPost(pRes.post);
        setEditPostForm({ title: pRes.post.title, content: pRes.post.content || "" });
        const cRes = await sendRequest(`${import.meta.env.VITE_BACKEND_URL}/api/comments/post/${postId}`);
        setComments(cRes.postComments || []);
      } catch (err) {}
    };
    if (postId) fetchData();
  ...
  // --- POST LOGIC HANDLERS ---
  const savePostEditHandler = async () => {
    if (editPostForm.title.trim().length === 0) return alert("Post title cannot be empty.");
    setIsSavingPost(true);
    try {
      const responseData = await sendRequest(
        `${import.meta.env.VITE_BACKEND_URL}/api/posts/${postId}/edit/texts`,
      ...
      setPost(responseData.updation.post);
      setIsEditingPost(false);
    } catch (err) {} finally { setIsSavingPost(false); }
  };
  ...
  const deletePostHandler = async () => {
    ...
    setIsDeleting(true);
    try {
      await sendRequest(`${import.meta.env.VITE_BACKEND_URL}/api/posts/${postId}`, "DELETE", null, { Authorization: `Bearer ${auth.token}` });
      navigate(`/forum/${post.forum}`);
  ...
  const changePostImageHandler = async (imageFile) => {
    ...
    try {
      const formData = new FormData();
      formData.append("image", imageFile);
      const responseData = await sendRequest(`${import.meta.env.VITE_BACKEND_URL}/api/posts/${postId}/edit/image`, "PATCH", formData, { Authorization: `Bearer ${auth.token}` });
      setPost({ ...post, image: responseData.updation.post.image });
      setShowImageMenu(false);
    } catch (err) {} finally { setIsUpdatingImage(false); }
  };

  const formatTimestamp = (dateString) => {
    if (!dateString) return null;
    return new Date(dateString).toLocaleString("en-US", { year: "numeric", month: "numeric", day: "numeric", hour: "numeric", minute: "numeric", hour12: true });
  };
  ...
  return (
    <div className="w-full py-4 md:py-8 max-w-5xl mx-auto px-4">
      {/* 1. BACK NAVIGATION */}
      <button onClick={() => navigate(-1)} className="flex items-center gap-2 text-zinc-500 hover:text-primary transition-colors mb-4 md:mb-6">
        <MdArrowBack className="text-xl" /> <span className="text-sm md:text-base">Back</span>
      </button>
      ...
      {post && (
        <div className="space-y-6">
          {/* 2. MAIN POST CARD */}
          <div className="bg-white rounded-xl border border-zinc-200 shadow-sm overflow-hidden flex flex-col md:flex-row relative">
            {/* Post Image Left/Top Container */}
            {post.image && (
              <div className="w-full md:w-1/3 bg-zinc-50 flex items-center justify-center p-4 md:p-6 border-b md:border-b-0 md:border-r border-zinc-200 relative">
                {post.creator?.id === auth.userId && (
                  <input type="file" ref={postImageInputRef} className="hidden" accept=".jpg, .jpeg, .png" onChange={(e) => { const file = e.target.files[0]; if (file) changePostImageHandler(file); }} />
                )}
                <div className={`relative w-full h-48 md:h-auto rounded-lg overflow-hidden group ${post.creator?.id === auth.userId ? "cursor-pointer" : ""}`} onClick={(e) => { e.stopPropagation(); if (post.creator?.id === auth.userId && !isUpdatingImage) setShowImageMenu(!showImageMenu); }}>
                  <img src={post.image} alt={post.title} className="w-full h-full object-contain" />
                  {post.creator?.id === auth.userId && (
                    <div className={`absolute inset-0 bg-black/25 flex items-center justify-center transition-opacity ${isUpdatingImage ? "opacity-100" : "opacity-0 group-hover:opacity-100"}`}>
                      {isUpdatingImage ? <div className="w-8 h-8 border-4 border-white border-t-transparent rounded-full animate-spin"></div> : <RiImageEditFill className="text-white text-3xl drop-shadow-lg" />}
                    </div>
                  )}
                </div>
              </div>
            ...
            {/* Post Content Right/Bottom Container */}
            <div className={`${post.image ? "md:w-2/3" : "w-full"} p-6 md:p-10 flex flex-col relative`}>
              {/* Management Dropdown for Post Owner */}
              {post.creator?.id === auth.userId && (
                <div className="absolute top-4 right-4 md:top-8 md:right-8 z-30 flex flex-col items-end" ref={postMenuRef}>
                  {isEditingPost ? (
                    <button onClick={savePostEditHandler} className="bg-primary text-white p-2 px-4 rounded-md flex items-center gap-2 hover:opacity-90 shadow-sm mb-2 text-sm md:text-base">
                      <MdSave className="text-lg md:text-xl" /> <span className="font-semibold">{isSavingPost ? "Saving..." : "Save"}</span>
                    </button>
                  ) : (
                    <button className={`p-2 rounded-full transition-colors ${isPostMenuOpen ? "bg-zinc-200 text-zinc-900" : "text-zinc-600 hover:bg-zinc-200/50"}`} onClick={() => setIsPostMenuOpen(!isPostMenuOpen)}><BsThreeDotsVertical className="text-lg md:text-xl" /></button>
                  )}
                  {isPostMenuOpen && (
                    <div className="absolute right-0 mt-2 w-44 md:w-48 bg-white border border-zinc-200 shadow-xl rounded-lg overflow-hidden animate-in fade-in zoom-in-95 duration-100">
                      <button className="w-full flex items-center gap-3 px-4 py-3 text-xs md:text-sm font-semibold text-zinc-700 hover:bg-zinc-50 border-b border-zinc-100" onClick={() => { setIsEditingPost(true); setIsPostMenuOpen(false); }}><BiSolidEditAlt className="text-lg text-zinc-500" /> Edit Text</button>
                      <button className="w-full flex items-center gap-3 px-4 py-3 text-xs md:text-sm font-semibold text-red-600 hover:bg-red-50" onClick={deletePostHandler}><MdDeleteForever className="text-lg md:text-xl" /> Delete Post</button>
                    </div>
                  )}
                </div>
              )}

              {/* View/Edit Toggle for Post Content */}
              {isEditingPost ? (
                <div className="space-y-6 mt-8 md:mt-0">
                  <input type="text" className="w-full bg-white border-b border-primary outline-none py-1 text-xl md:text-2xl font-bold" value={editPostForm.title} onChange={(e) => setEditPostForm({...editPostForm, title: e.target.value})} autoFocus />
                  <textarea ref={postContentRef} className="w-full bg-white border-b border-primary outline-none py-1 text-base md:text-lg text-zinc-700 resize-none" value={editPostForm.content} onChange={(e) => setEditPostForm({...editPostForm, content: e.target.value})} />
                  <button onClick={() => setIsEditingPost(false)} className="text-xs font-bold text-zinc-400 underline">Cancel</button>
                </div>
              ) : (
                <div className="mt-6 md:mt-0">
                  <h1 className="text-2xl md:text-3xl font-bold text-zinc-900 mb-4">{post.title}</h1>
                  <div className="text-zinc-700 text-base md:text-lg leading-relaxed whitespace-pre-wrap">{post.content}</div>
                </div>
              )}
            </div>
          </div>

          {/* 3. COMMENTS CHILD COMPONENT */}
          <Comments
            postId={postId}
            comments={comments}
